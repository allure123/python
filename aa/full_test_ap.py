...
def connect(basehost,access_token,device_mac,device_type="random",chip='0',auto=0):
    index=0
    while index<len(device_mac):
        url='http://'+basehost+'/gap/nodes/'+device_mac[index]+'/connection?chip='+str(chip)+'&mac='+apmac+''
        headers={'Authorization':'Bearer ' + access_token,'content-type':'application/json'}
        data={'timeout':'5000',"type":device_type,'auto':auto,'discovergatt':'1'}
        try:
        ...
        try:
            for line in r.iter_lines():                      
                ...
                if line.startswith('data'):
                    ...
                    # mac = ['11:22:11:22:11:33']
                    result=connect(basehost,access_token,device_mac=mac)
                    if result==200:
                        device_list+=get_condev(basehost, access_token)
...
if __name__=='__main__':
    ...
    t0=threading.Thread(target=scdwrd,name="scdwrd:",args=(basehost,),kwargs={'filter_uuid': '180d'})
