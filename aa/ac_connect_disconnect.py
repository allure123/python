        ...
        try:
            for line in r.iter_lines():
                ...
                if line.startswith('data'):
                    ...
                    result = connect(basehost, access_token, device_mac=mac, device_type="random")
                    if result == 200:
                        # print(device_list)
                        # time.sleep(5)
                        device_list += get_condev(basehost, access_token)
                        ...
                        if mac_scaned in device_list:
                            tag = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                            # print(tag + ': ' + 'Hold connection for more than 5s')
...
if __name__ == '__main__':
    device_mac = ['C0:00:5B:3F:76:CD']
    while True:
        connect(basehost, access_token, device_mac)
        time.sleep(20)
        try:
            disconnect(basehost, access_token, device_mac)
            time.sleep(20)
        except Exception as e:
            continue
        # get_condev(basehost, access_token)
        # time.sleep(10)
    # tag = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    # scdwrd(basehost, filter_mac='C7:F7:57:A9:FB:0E')
