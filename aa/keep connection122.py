...
basehost='10.10.10.254'
access_token=''
apmac=''
mutex = threading.Lock()


def get_info(basehost,access_token,key):    
    url_test='http://'+basehost+'/cassia/info/?mac='+apmac+''    
    if access_token:
        headers={'Authorization':'Bearer ' + access_token,'content-type':'application/json'}
        response=requests.request('GET', url=url_test,headers=headers)
    else:    
        url_login='http://'+basehost+'/cassia/login'+''
        login_data={'username':'admin','password':'`1q'}
...
def set_info(basehost,access_token,key,value):
    data={key:value}    
    url_test='http://'+basehost+'/cassia/info/?mac='+apmac+''
    if access_token:
        headers={'Authorization':'Bearer ' + access_token,'content-type':'application/json'}
        response=requests.request('POST', url=url_test,headers=headers,json=data)
    else:    
        url_login='http://'+basehost+'/cassia/login'+''
        login_data={'username':'admin','password':'`1q'}
        session=requests.Session()
    ...
    if response.status_code==200:
        result=response.content.decode('utf-8')
        # print('Set info: '+result)
    else:
        print('Set info failed: '+str(response.status_code))
        print(response.content.decode('utf-8'))

# requests.request has a problem, so change to use urllib3.PoolManage
def get_constatus(basehost,access_token):
...
def connect(basehost,access_token,device_mac,device_type="public",chip='0',auto=0):
    while True:
        url='http://'+basehost+'/gap/nodes/'+device_mac+'/connection?chip='+str(chip)+'&mac='+apmac+''
        headers={'Authorization':'Bearer ' + access_token,'content-type':'application/json'}
        data={'timeout':'5000',"type":device_type,'auto':auto,'discovergatt':'0', "random": "1"}
        try:
...
def discover(basehost,access_token,device_mac):
    ...
    if len(device_mac):
        url_all='http://'+basehost+'/gatt/nodes/'+device_mac[0]+'/services/characteristics/descriptors/?mac='+apmac+''                
        headers={'Authorization':'Bearer ' + access_token,'content-type':'application/json'}
        ...
        if response.status_code==200:
            ...
            if len(result_all):
                ...
                #             print('        char:'+result_all[i]['characteristics'][j]['uuid'])
                            if 'descriptors' in result_all[i]['characteristics'][j]:
                                for k in range(0,len(result_all[i]['characteristics'][j]['descriptors'])):
                                    descriptors_list.append(result_all[i]['characteristics'][j]['descriptors'][k]['uuid'])
                ...
                url_service='http://'+basehost+'/gatt/nodes/'+device_mac[0]+'/services/?mac='+apmac+'&all=1'
                ...
                for services_uuid in services_list:
                    url_service_uuid='http://'+basehost+'/gatt/nodes/'+device_mac[0]+'/services/?mac='+apmac+'&all=1&uuid='+services_uuid+''
                ...
                c=[]
                for services_uuid in services_list:
                    url_characteristics='http://'+basehost+'/gatt/nodes/'+device_mac[0]+'/services/'+services_uuid+'/characteristics?mac='+apmac+'&all=1'
                ...
                for characteristics_uuid in characteristics_list:
                    url_characteristics_uuid='http://'+basehost+'/gatt/nodes/'+device_mac[0]+'/characteristics?mac='+apmac+'&uuid='+characteristics_uuid
                ...
                d=[]
                for characteristics_uuid in characteristics_list:
                    url_descriptors='http://'+basehost+'/gatt/nodes/'+device_mac[0]+'/characteristics/'+characteristics_uuid+'/descriptors?mac='+apmac+''
    ...
    for mac in device_mac:
        ...
        if mac in device_list:            
            for handle in handle_list:
                url='http://'+basehost+'/gatt/nodes/'+mac+'/handle/'+str(handle)+'/value/?mac='+apmac+''
                headers={'Authorization':'Bearer ' + access_token,'content-type':'application/json'}
                try:
                ...
                if response.status_code==200:
                    ...
                    tag=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                    print(tag+': '+'Handle'+str(handle)+' read  failed: '+response.content.decode('utf-8')+'('+str(response.status_code)+')')            
...
def writeby_handle(basehost,access_token,device_mac,**handlee):
    ...
    for mac in device_mac:
        ...
        if mac in device_list:        
            for handle in handle_list:
                url='http://'+basehost+'/gatt/nodes/'+mac+'/handle/'+str(handle)+'/value/'+value+'/?mac='+apmac+''
                headers={'Authorization':'Bearer ' + access_token,'content-type':'application/json'}
                try:
                    response=requests.request('GET',url=url,headers=headers)
                except:
...
def scdwrd(basehost,access_token,active=0,chip=0,**filter_m):
        ...
        try:
            for line in r.iter_lines():                      
                ...
                if line.startswith('data'):
                    ...
                    tag=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())                
                    print('-------------------------------------------------------------------------')
                    print(tag+': '+'Scan test: Passed')
                    result=connect(basehost,access_token,device_mac=mac,chip=0)
                    if result==200:
                        device_list=get_condev(basehost,access_token)
                        if mac_scaned in device_list:
                            tag=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                            print(tag+': '+'Get connected device as a list test: Passed')
                            discover(basehost,access_token,device_mac=mac)
                            writeby_handle(basehost,access_token,device_mac=mac,value='0100',handle=[17])
                            writeby_handle(basehost,access_token,device_mac=mac,value='ff2006000227',handle=[19])
                            # writeby_handle(basehost,access_token,device_mac=mac,value='ff000f00040110080104583320522d',handle=[19])
                            readby_handle(basehost,access_token,device_mac=mac,handle=[17])                        
...
if __name__=='__main__':
    ...
    t0=threading.Thread(target=scdwrd,name="scdwrd:",args=(basehost,access_token),kwargs={'filter_uuid':'180d'})
    t1=threading.Thread(target=get_constatus,name="get_con_sta:",args=(basehost,access_token))
    t0.start()
    t1.start()
