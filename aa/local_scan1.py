...
def scan(basehost, access_token, time_scan=720000,active=0, chip=0, **filter_m):
    ...
    filter_value = filter_m['filter_value'] if ('filter_value' in filter_m) else ''
    message_num = 0
    message = ''
    ...
    try:
        url='http://'+basehost+'/gap/nodes?event=1&mac='+apmac+'&chip='+str(chip)+'&active='\
             ...
             +filter_mac+'&filter_uuid='+filter_uuid+'&filter_duplicates='+filter_duplicates+'&filter_value='\
             +filter_value+''
        print(url)
...
if __name__=='__main__':
    ...
    # print(access_token)
    scan(basehost, access_token, active=1, chip=1)
    # print('{"data": "0d18"}')
